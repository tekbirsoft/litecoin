Husancoin Core version 0.13.2 is now available from:
...
ECDSA signatures inside Husancoin transactions now use validation using
...
Previous versions of Husancoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Husancoin Core was to raise the default minimum
relay fee.

Husancoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Husancoin Core 0.13.2 also introduces new default policy limits on the length and
...
Husancoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Husancoin Core will relay transactions with
...
entirely in Husancoin Core 0.13, and it is at this time undecided whether the
...
Husancoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Husancoin Core automatically creates a hidden service to listen on, without
manual configuration. Husancoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Husancoin Core is listening, and
...
Husancoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Husancoin
Core to use floating fees. By default, Husancoin Core will use floating
fees.
...
At all times, Husancoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) HSC.
Furthermore, Husancoin Core will never create transactions paying less than
...
Various code modernizations have been done. The Husancoin Core code base has
...
for a new opcode in the Husancoin scripting system that in combination with
...
Husancoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Husancoin Core.
...
The two phases are distinct in the Husancoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Husancoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Husancoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Husancoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Husancoin addresses (both P2PKH addresses
  ...
  believed to be provided by Husancoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Husancoin's current ledger (called the UTXO set),
  ...
  note that Husancoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Husancoin users to individually opt-in to almost any change in the Husancoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Husancoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Husancoin
client to Husancoin Core 0.13.2 to signal for segwit support. When a
...
Since Husancoin Core 0.10.0, nodes have defaulted to only relaying and
