...
const char * const BITCOIN_CONF_FILENAME = "husancoin.conf";
const char * const BITCOIN_PID_FILENAME = "husancoin.pid";
...
#else
    const char* pszModule = "husancoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Husamcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Husamcoin";
#else
    // Unix
    return pathRet / ".husancoin";
#endif
#endif
}
